...
각요소를 arr 변수에 넘기고 내부의 코드를 실행해서 
return 하면 새로운 배열을 만드는 코드
기존의 배열을 변경하지 않고, 어떤 연산을 수행한후 
똑같은 개수의 배열을 만들어 준다
...
const mapArr = myarray.map((arr) => {
  //arr++ // map 함수내부에서 myarray의 요소를 변경하는 코드는 가급적 지양 하라
...
/*
slice(시작,종료) : 시작위치로 부터 종료-1 위치까지 요소를 자르기
*/
...
*/
const phon = ["홍길동", "010-111", 32];
let user = {};
[user.name, user.number, user.age] = phon;
console.log(user);
